...
│   ├── copyable-code-block.tsx     # Terminal-style command block with copy button + syntax highlighting
│   ├── highlighted-code-text.tsx   # Token-level syntax highlighting for shell, prompt, and output text
...
│   ├── skill-card-client.tsx       # Compact 2-col skill rows, shared runtime picker, "see example" disclosure + typewriter
...
- **The skills page has a unified install card** — `RuntimeInstallCard` in `src/components/skill-card-client.tsx` combines 6 runtime tabs (Any, Claude Code, OpenClaw, Codex CLI, OpenCode, Cursor) with a copyable code snippet in one bordered card. Claude Code is the default runtime. Per-skill rows use standalone `CopyableCodeBlock` components and no longer simulate local install/remove state.
- **Code blocks use token-level syntax highlighting** — `HighlightedCodeText` (`src/components/highlighted-code-text.tsx`) tokenizes shell commands and output into semantic spans (flags, paths, URLs, numbers, addresses, env vars, etc.) with muted color coding. Used in `CopyableCodeBlock` and the typewriter example output.
- **The skills page is a compact two-column layout** — each skill is a numbered row with description + install command on the left and a "see example" disclosure on the right. Execution skills sort first. One shared runtime picker updates every command block. The example panel is a placeholder button that expands into a typewriter terminal output on click. IntersectionObserver gates the typewriter so it only animates when the row is 30% visible. No nested accordions.
...
- **Activity ticker is live** — the marquee server-renders initial `/api/activity` data, refreshes on the client every 30 seconds, and keeps production honest with loading, empty, and unavailable states instead of fabricated fallback trades. The ticker measures real rendered width via `useLayoutEffect` + `ResizeObserver` (remeasures on resize, font load, and DOM size changes) and keeps a conservative minimum number of repeated copies mounted so a refetch or shorter payload can't momentarily underfill the track. A blinking cursor follows every entry. Amounts prefer USD (`$142`, `$1.2K`), fall back to native currency (`0.0001 ETH`, `12.22 USDC`) when USD is unavailable, and show `<$1` for nonzero sub-dollar values. The feed fetches 2x the requested count and deduplicates by `(trader, action, coin)` to avoid visually repeated entries.
...
- **Wallet connect is address-only** — the modal at `src/components/wallet-connect-modal.tsx` accepts a wallet address from `zora wallet` and stores it locally. No signatures, nonces, or browser-side verification flow are involved because portfolio data is public.
